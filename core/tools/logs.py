...
class Log:
    ...
    def _write(self, count_episodes, count_states, msg):
        ...
        with open(file=self.file_path, mode='a', buffering=1) as file:
            msg_annotated = f"{count_episodes}\t{count_states}\t{msg}\n"
...
class LogReward(Log):
    ...
    def write(self, count_episodes, count_states, reward_episode, reward_mean):
        ...
        msg = "{episode:.1f}\t{mean:.1f}".format(episode=reward_episode, mean=reward_mean)
...
class LogQValues(Log):
    ...
    def write(self, count_episodes, count_states, q_values):
        ...
        msg = "{min:.3f}\t{mean:.3f}\t{max:.3f}\t{std:.3f}".format(min=np.min(q_values),
    ...
    def read(self):
        ...
        self.min, self.mean, self.max, self.std = self.data
